...
class CountdownViewController: UIViewController {
    @IBOutlet weak var countdownPickerView: CountdownPicker!   //this got changed to access
    ...
    @IBAction func startButtonTapped(_ sender: Any) {
        //self.showAlert()
  //      let timer = Timer.scheduledTimer(withTimeInterval: 2, repeats: false, block: timerFinished(time:))
...
extension CountdownViewController: CountdownDelegate {
    ...
    private func updateViews() {
        ...
        //default value is not needed here because we call them all - .started, .finished, .reset
        }
    }
}
